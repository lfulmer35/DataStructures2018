...
void WaterVehicle::setStartLocation(Grid& _grid, WaterVehicle& _ship)
{
	...
	_ship.setOrientation(_grid, _ship);
}
void WaterVehicle::setOrientation(Grid& _grid, WaterVehicle& _ship)
{
	char _orientation;
	bool check = false;
...
void WaterVehicle::checkSunk(const Grid& _grid, WaterVehicle& _carrier, WaterVehicle& _battleship, WaterVehicle& _cruiser, WaterVehicle& _submarine, WaterVehicle& _destroyer)
{
	...
	for (int i = 0; i < _grid.cols; i++)
	{
		...
		{
			if (_grid.userVec[i][x] == 'C')
				_carrier.isSunk = false;
			else if (_grid.userVec[i][x] == 'B')
				_battleship.isSunk = false;
			else if (_grid.userVec[i][x] == 'c')
				_cruiser.isSunk = false;
			else if (_grid.userVec[i][x] == 'S')
				_submarine.isSunk = false;
			else if (_grid.userVec[i][x] == 'D')
				_destroyer.isSunk = false;
		}
	}
	if (_carrier.isSunk == true)
		cout << "Carrier was sunk." << endl;
	else if (_battleship.isSunk == true)
		cout << "Battleship was sunk." << endl;
	else if (_cruiser.isSunk == true)
		cout << "Cruiser was sunk." << endl;
	else if (_submarine.isSunk == true)
		cout << "Submarine was sunk." << endl;
	else if (_destroyer.isSunk == true)
		cout << "Destroyer was sunk." << endl;
	else
		cout << "No ships were sunk." << endl;
}
void WaterVehicle::userTorpedo()
{
	//this function will allow the user to fire his own torpedoes at a computer opponent

}
void WaterVehicle::randomTorpedo(Grid& _grid)
{
	//This function fires 15 random torpedoes and counts the number of hits
	int hits = 0;
	for (int i = 0; i < 15; i++)
	{
		int randRow = rand() % 10;
		int randCol = rand() % 10;
		if (_grid.userVec[randRow][randCol] == 'O')
			_grid.userVec[randRow][randCol] = 'X';
		else if (_grid.userVec[randRow][randCol] == 'S')
		{
			_grid.userVec[randRow][randCol] = 'X';
			hits += 1;
		}
	...
	_grid.displayVec(_grid);
	cout << hits << " torpedoes struck your ships!!" << endl << endl;
}
